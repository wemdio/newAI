...
export const buildAnalysisPrompt = (message, userCriteria) => {
  ...
  // Emphasize user criteria importance
  const prompt = `КРИТЕРИИ ПОЛЬЗОВАТЕЛЯ (следуй точно, ОСОБЕННО секцию "НЕ СЧИТАТЬ ЛИДОМ"):
${userCriteria}
...
ЗАДАЧА: 
1. Определи направление: человек ИЩЕТ услугу или ПРЕДЛАГАЕТ услугу?
   - Если ПРЕДЛАГАЕТ → is_match: false (не лид)

2. Если ИЩЕТ, проверь ТЕМАТИЧЕСКОЕ СООТВЕТСТВИЕ:
   - Прочитай конкретные темы/услуги/проблемы в КРИТЕРИЯХ ПОЛЬЗОВАТЕЛЯ
   - Соответствует ли ТЕМА сообщения этим конкретным темам?
   - Не интерпретируй слишком широко! Если человек ищет А, а в критериях Б - это НЕ лид

3. Проверь секцию "НЕ СЧИТАТЬ ЛИДОМ"

ВАЖНО: is_match: true только если совпадает И направление (ищет) И тема (конкретно указанная в критериях)
